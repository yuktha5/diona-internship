/**
 * Dynamic Document Rendering & Smart Pagination Engine
 */
...
const DocumentRenderer = {
  ...
  renderPrivacyConsent(isChecked) {
    return `
      <div class="privacy-consent-block">
        <div class="privacy-checkbox ${isChecked ? 'checked' : ''}"></div>
  ...
  paginateAndRender(data, targetContainer) {
    ...
    // Collect all modular block components
    const blocks = [];
    ...
    if (data.busOrTaxi && data.busOrTaxi.length > 0) {
      blocks.push({
        ...
        forceNewPageIfExact: true // Matches PDF convention where Bus/Taxi begins Page 2 in standard document
      });
    }
    ...
    // Page budget in weight units (Letter page ~ 720 units)
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      // Check if this block triggers a page break (e.g. Bus/Taxi section in default replica or weight overflow)
    ...
    // Render each page DOM node
    pages.forEach((pageBlocks, index) => {
      const pageNum = index + 1;
      const pageEl = document.createElement('div');
      pageEl.className = `pdf-page page-${pageNum}`;
...
// Export for global or module environments
